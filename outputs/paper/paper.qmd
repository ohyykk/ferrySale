...
dir.create(here::here("STA304/paper1/ferrySale/outputs"), showWarnings = FALSE, recursive = TRUE)

# Save the dataset to a CSV file
write_csv(marriage_licence_statistics, here::here("STA304/paper1/ferrySale/outputs/a.csv"))

# Read the dataset back
data <- read_csv(here::here("STA304/paper1/ferrySale/outputs/a.csv"))
...
# Plot Redemption and Sales Counts Over Time
...
ggplot(data, aes(x = Redemption.Count, y = Sales.Count)) +
  geom_point(color = "purple", size = 3, alpha = 0.7) +  # Add transparency for overlapping points
  labs(title = "Relationship Between Redemption and Sales Count",
       subtitle = "Scatter plot with Redemption Count on X-axis and Sales Count on Y-axis",
       x = "Redemption Count (units)",
       y = "Sales Count (units)",
       caption = "Source: Company Sales Data") +  # Add a caption for clarity
  ...
  theme(
    ...
    axis.title.x = element_text(face = "italic", margin = margin(t = 10)),  # Italicize X axis label and add margin
    axis.title.y = element_text(face = "italic", margin = margin(r = 10)),  # Italicize Y axis label and add margin
    panel.grid.major = element_line(color = "gray80", size = 0.5),  # Make grid lines more subtle
    panel.grid.minor = element_blank(),  # Remove minor grid lines
    plot.caption = element_text(hjust = 1, face = "italic", color = "gray40")  # Style caption
  )
